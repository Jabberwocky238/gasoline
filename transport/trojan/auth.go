...
func (a *Authenticator) AuthUser(hash string, addr string) (bool, *User) {
...
func (a *Authenticator) GetUser(hash string) (*User, error) {
	if user, found := a.Users.Load(hash); found {
...
func NewAuthenticator(ctx context.Context, passwords []string) *Authenticator {
	u := &Authenticator{
		Ctx: ctx,
	}
